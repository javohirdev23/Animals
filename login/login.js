...
const elLoginBtn = document.querySelector(".js-login-btn");
elLogForm.addEventListener("submit", (evt) => {
  evt.preventDefault();
...
`;
  const formData = new FormData(elLogForm);
  const result = {};
...
function login(data) {
  fetch("https://json-api.uz/api/project/game-over/auth/login", {
    ...
    .catch(() => {
      alert("Sorry somthing wrong place check your username and password!");
    })
    .finally(() => {});
}
